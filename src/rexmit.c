 ...
 */
#define tv_ge(lhs,rhs) (time_cmp((lhs),(rhs)) >= 0)
#define tv_gt(lhs,rhs) (time_cmp((lhs),(rhs)) >  0)
#define tv_le(lhs,rhs) (time_cmp((lhs),(rhs)) <= 0)
#define tv_lt(lhs,rhs) (time_cmp((lhs),(rhs)) <  0)
#define tv_eq(lhs,rhs) (time_cmp((lhs),(rhs)) == 0)
...
static void insert_seg_between(struct quadrant *, struct segment *, struct segment *, struct segment *);
static struct segment *create_seg(uint32_t, u_long, u_short);
static struct quadrant *whichquad(struct seqspace *, uint32_t);
static void collapse_quad(struct quadrant *);
static struct quadrant *create_quadrant(void);
static int addseg(struct end_to_end *, struct quadrant *, uint32_t, u_long, Bool *, u_short);
static void rtt_retrans(struct end_to_end *, struct segment *);
static enum t_ack rtt_ackin(struct end_to_end *, struct segment *, Bool rexmit);

/* LM start return the type of struct segment */
void rules_test(struct end_to_end *, struct segment *, u_long, struct quadrant *, u_short,
		Bool pkt_already_seen, double recovery_time);
char real_rules_test(struct end_to_end * thisdir, struct segment * pseg, u_long len,
		     struct quadrant * pquad, u_short this_ip_id,
		     Bool pkt_already_seen, double *recovery_time);
/* LM stop*/
 ...
 */
int
rexmit(struct end_to_end * ptcb, uint32_t seq, u_long len, Bool * pout_order,
       u_short this_ip_id)
{
	struct seqspace *sspace = &(ptcb->ss);
	uint32_t seq_last = seq + len - 1;
	...
	*pout_order = FALSE;
	...
	if (BOUNDARY(seq, seq_last)) {
		...
		rexlen =
		    addseg(ptcb, pquad, seq1, len1, pout_order, this_ip_id);

		/* in second quadrant */
		seq2 = FIRST_SEQ(QUADNUM(seq_last));
		len2 = len - len1;
		rexlen +=
		    addseg(ptcb, pquad->next, seq2, len2, pout_order,
			   this_ip_id);
	} else {
...
static int
addseg(struct end_to_end * ptcb,
       struct quadrant * pquad,
       uint32_t thisseg_firstbyte, u_long len, Bool * pout_order,
       u_short this_ip_id)
{
	uint32_t thisseg_lastbyte = thisseg_firstbyte + len - 1;
	struct segment *pseg;
	struct segment *pseg_new;
	int rexlen = 0;
	Bool split = FALSE;
	...
	for (; pseg != NULL; pseg = pseg->next) {
		...
		if (thisseg_firstbyte < pseg->seq_firstbyte) {
			...
			pseg_new =
			    create_seg(thisseg_firstbyte, len, this_ip_id);
			...
			if (thisseg_lastbyte <= pseg->seq_firstbyte) {
				...
				// LM start
				rules_test(ptcb, pseg, len, pquad, this_ip_id,
					   FALSE, recovery_time);
				return (rexlen);
			} else {
				...
				pseg_new->seq_lastbyte =
				    pseg->seq_firstbyte - 1;

				// LM start
				rules_test(ptcb, pseg,
					   pseg_new->seq_lastbyte -
					   pseg_new->seq_firstbyte, pquad,
					   this_ip_id, FALSE, recovery_time);
		...
		if (thisseg_firstbyte >= pseg->seq_firstbyte) {
			...
			++pseg->retrans;
			recovery_time =
			    TIME2DOUBLE(tct_tm.current_time) - TIME2DOUBLE(pseg->time);
			if (!split)
	...
	if (pquad->no_of_segments > MAX_SEG_PER_QUAD) {
		...
		struct segment *tmp_pseg = pquad->seglist_head;
...
/**********************************************************************/
static struct segment *create_seg(uint32_t seq, u_long len, u_short this_ip_id)
{
	struct segment *pseg;

	pseg = (struct segment *) segment_alloc();

	pseg->time = tct_tm.current_time;
...
static struct quadrant *create_quadrant(void)
{
	...
	pquad = (struct quadrant *) quadrant_alloc();
	return (pquad);
}

/********************************************************************/

static struct quadrant *whichquad(struct seqspace * sspace, uint32_t seq)
{
	u_char qid = QUADNUM(seq);
	struct quadrant *pquad;
	int qix;
	int qix_next;
	int qix_opposite;
	...
	if (sspace->pquad[qix_opposite] != NULL) {
		free_quad(&sspace->pquad[qix_opposite]);
...
/*********************************************************************/
static void
insert_seg_between(struct quadrant * pquad,
		   struct segment * pseg_new,
		   struct segment * pseg_before, struct segment * pseg_after)
{
...
/*********************************************************************/
static void collapse_quad(struct quadrant * pquad)
{
	Bool freed;
	struct segment *pseg;
	...
	while (pseg != NULL) {
		...
		if (pseg->acked && pseg->next->acked &&
			...
			if (tv_gt(pseg->next->time, pseg->time)) {
				...
				pseg->type_of_segment =
				    pseg->next->type_of_segment;
			}
...
static void rtt_retrans(struct end_to_end * ptcb, struct segment * pseg)
{
	double etime;
	...
	if (!pseg->acked) {
		...
		etime = elapsed(pseg->time, tct_tm.current_time);
...
static enum t_ack rtt_ackin(struct end_to_end * ptcb, struct segment *pseg, Bool rexmit_prev)
{
	double etime_rtt;
	enum t_ack ret;

	/* how long did it take */
	etime_rtt = elapsed(pseg->time, tct_tm.current_time);
//fprintf (fp_stdout, "%f\n",etime_rtt);
	if (rexmit_prev) {
		...
		/* see RFC 2988 */
		ptcb->rttvar =
		    (1.0 - BETA) * ptcb->rttvar + BETA * abs(ptcb->srtt -
							     etime_rtt);
...
{
	struct quadrant *pquad;
	struct quadrant *pquad_prev;
	struct segment *pseg;
	Bool changed_one = FALSE;
	Bool intervening_xmits = FALSE;
	struct timeval last_xmit = { 0, 0 };
	int ret = 0;
  ...
  */
char
real_rules_test(struct end_to_end * thisdir, struct segment * pseg, u_long len, struct quadrant * pquad,
		...
		double *recovery_time)
{
	...
	int dir = (&(thisdir->parent->c2s) == thisdir);

	otherdir = (dir == C2S) ? &(thisdir->parent->s2c) : &(thisdir->parent->c2s);
	Mean_RTT = average(thisdir->rtt_sum, thisdir->rtt_count) + 
		average(otherdir->rtt_sum, otherdir->rtt_count);
	RTO =
	    Mean_RTT +
	    4 *
	    (stdev
	...
	if (!pkt_already_seen)	/* if pkt_already_seen then *recovery_time is passed otherwise it is set below */
		*recovery_time =
		    (pseg->prev->prev !=
		     NULL) ? TIME2DOUBLE(tct_tm.current_time) -
	...
	if (pseg->prev != NULL) {
		if (pkt_already_seen)
			...
			prev_tos =
			    (pseg->prev->prev !=
			     NULL) ? pseg->prev->
			    prev->type_of_segment : IN_SEQUENCE;
	} else
	...
	if (pkt_already_seen)
		...
		Rule2b =
		    (pseg->prev->prev != NULL) ? (pseg->prev->prev->acked > 3
						  && *recovery_time < RTO) : 0;

	Rule2c = (TIME2DOUBLE(tct_tm.current_time) - TIME2DOUBLE(pseg->time) < RTT);

	RuleProbing = ((len == 1) && (otherdir->win_curr == 0)
		       && (thisdir->syn_count != 0)
		       && (otherdir->syn_count != 0));
	...
	if (!pseg->acked) {
		if (pkt_already_seen) {
			...
			if (Rule1d)
				return
				    CLASSIFICATION
				    (DUPLICATE_WITH_RC_LESS_THAN_RTT_NOT_3DUP_ACK);
			if (!Rule1b)
				return
				    CLASSIFICATION
		...
		if (Rule1d)
			return
			    CLASSIFICATION
			    (OOO_WITH_RC_LESS_THAN_RTT_NOT_3DUP_ACK);
		if (!Rule1b)
			return
			    CLASSIFICATION
...
void
rules_test(struct end_to_end * thisdir, struct segment * pseg, u_long len, struct quadrant * pquad,
	   u_short this_ip_id, Bool pkt_already_seen, double recovery_time)
{
	double DeltaT2, RTO, Mean_RTT, RTT_min;
	char type_of_segment =
	    real_rules_test(thisdir, pseg, len, pquad, this_ip_id,
			    pkt_already_seen,
			    &recovery_time);
	struct end_to_end *otherdir;
	int dir, num_acked;
	...
	if (pkt_already_seen) {
		...
		num_acked =
		    (pseg->prev->prev != NULL) ? pseg->prev->prev->acked : 0;
	}
	/* LM added */

	dir = (&(thisdir->parent->c2s)) == thisdir;
	otherdir = (dir == C2S) ? &(thisdir->parent->s2c) : &(thisdir->parent->c2s);

	DeltaT2 =
	    TIME2DOUBLE(tct_tm.current_time) - TIME2DOUBLE(pquad->seglist_tail->time);
	...
	if (type_of_segment != 0) {
		wfprintf(fp_dup_ooo_log, "T: %f ",
			 (elapsed(first_packet, tct_tm.current_time) / 1000.0));
		if (dir == C2S) {
			wfprintf(fp_dup_ooo_log, "%s %s ",
				 HostName(thisdir->parent->addr_pair.a_address),
				 ServiceName(thisdir->parent->addr_pair.a_port));
			wfprintf(fp_dup_ooo_log, "%s %s ",
				 HostName(thisdir->parent->addr_pair.b_address),
				 ServiceName(thisdir->parent->addr_pair.b_port));
		} else {
			wfprintf(fp_dup_ooo_log, "%s %s ",
				 HostName(thisdir->parent->addr_pair.b_address),
				 ServiceName(thisdir->parent->addr_pair.b_port));
		...
		wfprintf(fp_dup_ooo_log, " %f %f %f %f\n",
			 thisdir->srtt / 1000.0, thisdir->rttvar / 1000.0,
			 otherdir->srtt / 1000.0, otherdir->rttvar / 1000.0);
	}
#endif
